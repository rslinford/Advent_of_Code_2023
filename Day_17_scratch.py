...
def day17(filepath, part2=False):
    ...
    dirs = {'r': (0, 1),
            ...
            'u': (-1, 0)}
    grid = Grid2d(lines)
    h = [(0, (0, 0), ('r', 0))]  # heap (heat loss, (x, y), (current direction, heading_step taken in that direction)
    if part2:
            ...
            if w:
                ...
                if part2:
                    if (d == hd and hs == 10) or opposites(d, hd):
                        # skip this heading if we have taken the 10 heading_step already
                        continue
                    if hs < 4 and d != hd:
                        continue
                else:
                    # check to see if we have done three heading_step in that direction already
                    # or if the direction is the opposite of the current heading
                    if (d == hd and hs == 3) or opposites(d, hd):
                        # skip this heading if we have taken the 3 heading_step already
                        continue
...
def main():
    # assert day17('Day_17_input.txt') == 102
    print(f"Part 1: {day17('Day_17_input.txt')}")

    # assert day17('Day_17_input.txt', True) == 94
    print(f"Part 2: {day17('Day_17_input.txt', True)}")
